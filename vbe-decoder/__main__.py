...
def decode_file(file :str):
    try:
        with open(file, 'rb') as handle:  # Open the file in binary mode
            contents = handle.read().decode('latin-1')  # Decode using 'latin-1' to avoid Unicode errors
...
def main():
    ...
    output = decode_files(args.files)
